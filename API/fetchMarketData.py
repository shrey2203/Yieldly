...
async def scrappingData_async(allTradedEquities):
    ...
    start = time.time()
    results, equitiesRem = nseEqOptimised(allTradedEquities)
    for symbol, result in results.items():
        try:
            print ("[" + time.strftime("%Y-%m-%d %H-%M-%S") + "] " + "Fetching latest market data via index for : " + symbol)
            equitiesData[symbol]['lastPrice'] = result['lastPrice']
            equitiesData[symbol]['Industry'] = result['industry']
            equitiesData[symbol]['52w Low'] = result['yearHigh']
            equitiesData[symbol]['52w High'] = result['yearLow']
            equitiesData[symbol]['dailyChangePercent'] = result['dailyPChange']
            equitiesData[symbol]['dailyChange'] = result['dailyChange']
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        with equitiesLock:  # Lock while reading equities list
            equitiesRemLocked = list(equitiesRem) 
        tasks = [getQuote_async(executor, equity) for equity in equitiesRemLocked]
    ...
    for result in results:
        if result and 'info' in result:
            try:
                ...
                print ("[" + time.strftime("%Y-%m-%d %H-%M-%S") + "] " + "Fetching latest market data via quote for : " + symbol)
