...
if(mode=='local'):
    ...
    def nsefetch(url, max_retries=3):
        session = requests.Session()

        # Step 1: Establish session by visiting the NSE homepage
        homepage = "https://www.nseindia.com/"
        print("Fetching NSE homepage to establish session...")
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Debugging: Log response status and first 500 characters
                print(f"Attempt {attempt + 1}: Status Code = {response.status_code}")
                print("Response Text (first 500 chars):", response.text[:500])

                # Step 2: If response is HTML (not JSON), retry after visiting homepage
                if "<html" in response.text:
                    print("Warning: Received HTML response. Retrying...")
                    session.get(homepage, headers=HEADERS, timeout=10)  # Refresh session
                    time.sleep(2)
                    continue
        ...
        print("Final Error: NSE API is not returning JSON data.")
        return None
...
def nse_eq(symbol):
    ...
    try:
        ...
        try:
            if(payload['error']=={}):
                print("Please use nse_fno() function to reduce latency." + symbol)
                payload = nsefetch('https://www.nseindia.com/api/quote-derivative?symbol='+symbol)
        except:
            pass
    except KeyError:
...
def nseGetStocksInIndex(index):
    payload = nsefetch('https://www.nseindia.com/api/equity-stockIndices?index=' + index)
    return payload

def def_value():
    return defaultdict()

def nseEqOptimised(equities):
    equitiesData = defaultdict(def_value)
    for index in ("NIFTY 200", "NIFTY MIDCAP 150", "NIFTY SMALLCAP 250"):
        try:
            payload = nsefetch('https://www.nseindia.com/api/equity-stockIndices?index=' + index)
            for item in payload['data']:
                    if any(eq == item['symbol'] for eq in equities):
                        equitiesData[item['symbol']] = {}
                        equitiesData[item['symbol']]["Open"] = item['open']
                        equitiesData[item['symbol']]["dayHigh"] = item['dayHigh']
                        equitiesData[item['symbol']]["dayLow"] = item['dayLow']
                        equitiesData[item['symbol']]["lastPrice"] = item['lastPrice']
                        equitiesData[item['symbol']]["dailyChange"] = item['change']
                        equitiesData[item['symbol']]["dailyPChange"] = item['pChange']
                        equitiesData[item['symbol']]["yearHigh"] = item['yearHigh']
                        equitiesData[item['symbol']]["yearLow"] = item['yearLow']
                        equitiesData[item['symbol']]["industry"] = item['meta']['industry']
        except KeyError:
            print("Getting Error While Fetching.")
    equitiesRem = list(set(equities) - set(equitiesData.keys()))
    return equitiesData, equitiesRem
    # for symbol in equities:
    #     print ("Fetching market seperately for the symbol : " + symbol)
    #     symbol = nsesymbolpurify(symbol)
    #     try:
    #         payload = nsefetch('https://www.nseindia.com/api/quote-equity?symbol='+symbol)
    #         try:
    ...
    #         print("Getting Error While Fetching.")
    #     equitiesData[symbol] = {}
    #     equitiesData[symbol]["Open"] = payload['priceInfo']['open']
    #     equitiesData[symbol]["dayHigh"] = payload['priceInfo']['intraDayHighLow']['max']
    #     equitiesData[symbol]["dayLow"] = payload['priceInfo']['intraDayHighLow']['min']
    #     equitiesData[symbol]["lastPrice"] = payload['priceInfo']['lastPrice']
    #     equitiesData[symbol]["dailyChange"] = payload['priceInfo']['change']
    #     equitiesData[symbol]["dailyPChange"] = payload['priceInfo']['pChange']
    #     equitiesData[symbol]["yearHigh"] = payload['priceInfo']['weekHighLow']['max']
    #     equitiesData[symbol]["yearLow"] = payload['priceInfo']['weekHighLow']['min']
    #     equitiesData[symbol]["industry"] = payload['info']['industry']
    #     equities.remove(symbol)
    # if len(equities) != 0: print ("Count of scrip for which market data could not be fetched was : " + str(len(equities)))
    # return equitiesData
