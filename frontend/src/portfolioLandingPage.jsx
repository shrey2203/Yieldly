...
const Portfolio = () => {
    ...
    const fetchTotalDividends = useCallback(async () => {
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(
                `/api/fetchTotalDividends?userId=${userId}`, 
                {headers: { 'Authorization': `${token}` }}
            );
            if (!response.ok) throw new Error("Network response was not ok");
            const data = await response.json();
    ...
    // Scenario B: Clicked from Holdings Tab (Full Object already exists)
    else if (!params.data.transactionSummary) {
        const fullScripData = portfolioData.find(item => item.stock === params.data.stock);
        if (fullScripData) dataToDisplay = fullScripData;
    }
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const totalValue = cleanData.reduce((sum, item) => sum + (Number(item.totalValue) || 0), 0);
    
            // Update Holdings
            setPortfolioData(cleanData.map(item => ({
                ...item,
                portfolioWeight: totalValue ? ((Number(item.totalValue) / totalValue) * 100) : 0
            })));
    ...
    }, [portfolioData, rawRealisedData, totalDividends]); // Depend on the raw state
    ...
    const columnDefs = useMemo(() => [
        ...
        { 
            ...
            flex: 0.9, width: 80, minWidth: 70, cellClass: "avg-highlight",
            valueFormatter: p => formatINR(p.value) 
        },
        { 
