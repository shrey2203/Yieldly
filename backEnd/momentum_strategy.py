"""
Standalone Momentum & Trend Strategies Module
---------------------------------------------
Provides standalone, plug-and-play quantitative trading strategies:
1. EmaMomentumStrategy: 20/50/200 EMA Trend & Volume Expansion
2. SupertrendStrategy: Supertrend (10, 3) & Volatility Breakout
"""
...
def compute_supertrend(df: pd.DataFrame, period: int = 10, multiplier: float = 3.0):
    ...
    direction = np.ones(len(df))  # 1 = Bullish / Green, -1 = Bearish / Red
...
class EmaMomentumStrategy:
    ...
    def run_backtest(cls, stock: str, lookback_years: int = 2, initial_capital: float = 100000.0) -> Dict[str, Any]:
        try:
            ...
            for suffix in [".NS", ".BO"]:
                try:
                    raw = yf.download(f"{stock}{suffix}", period=f"{lookback_years}y", interval="1d", progress=False, auto_adjust=True)
            ...
            for i in range(start_idx, len(c_arr)):
                ...
                if not in_pos:
                    # Entry: Close > 200 EMA, 20 EMA > 50 EMA, near 20 EMA or 20 EMA crossed above 50 EMA, vol > 1.25x, RSI 44-65
                    ...
                    # Profit milestone trailing
                    gain_pct = ((c - entry_p) / entry_p) * 100
                    if gain_pct >= 6.0 and not trailing_be:
                        stop_p = max(stop_p, entry_p * 1.005)  # Trail to Breakeven
                        trailing_be = True
                    if gain_pct >= 10.0:
                        stop_p = max(stop_p, entry_p * 1.05)  # Lock in 5%
                    ...
                    # Breakdown below 50 EMA
                    elif c < e50 * 0.98 and (i - entry_i) >= 4:
                        exit_p = c
                        exit_reason = f"50 EMA Breakdown"
                    # Time exit (40 days max holding)
                    elif (i - entry_i) >= 40:
                        exit_p = c
            ...
            # Sample equity curve to 60 points for light UI rendering
...
class SupertrendStrategy:
    ...
    def generate_signal(cls, df: pd.DataFrame, current_price: float) -> Dict[str, Any]:
        ...
        # Fresh bull flip or sustained bull
        if curr_dir == 1:
        ...
        try:
            ...
            for i in range(start_idx, len(c_arr)):
                ...
                if not in_pos:
                    ...
                    stop_p = max(stop_p, st)  # Ratchet stop up
                    exit_reason = None
                    exit_p = c

                    if curr_dir == -1 or l <= stop_p:
                        exit_p = min(c, stop_p) if l <= stop_p else c
